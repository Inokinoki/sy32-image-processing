...
def get_data(path, classes, img_width, img_height):
    ...
    # for c in classes:
    for r, d, fs in os.walk(os.path.join(path, "pos")): #"train-data/train/pos"):
        for f in fs:
            pos_images.append(util.img_as_float(io.imread(r + "/" + f)))
    for r, d, fs in os.walk(os.path.join(path, "neg")):
        for f in fs:
...
if __name__ == "__main__":
    ...
    train_x, train_y = get_data("train-data/train", ["pos", "neg"], 24, 24)
