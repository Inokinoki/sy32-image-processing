...
def apply_window(image, model, preprocess, hstep=1, wstep=1):
    if image.shape[0] < int(HEIGHT) or image.shape[1] < int(WIDTH):
        return []
    matched_points,scores,positions,preprocessed_datas  = list(),list(),list(),list()
...
def generate_detction_txt(labels,number_of_args,image_index):
    if number_of_args <=0:
        file_decision = open("decision.txt", "w")
        for label in labels:
               file_decision.write("%03d %d %d %d %d %.2f\n" % label)
        file_decision.close()
    else:
        f = open("test-%04d.txt" % image_index, "w")
        for label in labels:
    ...
    for image_index in image_indexs:
        ...
        for image_pyramid, i in zip(images_pyramid, range(len(images_pyramid))):
            if image_pyramid.shape[0] > HEIGHT and image_pyramid.shape[1] > WIDTH:
                hstep=int(round(max(4 / pow(SCALE, i), 1)))
                wstep=int(round(max(3 / pow(SCALE, i), 1)))
                positions, scores = apply_window(image_pyramid, model, hog_feature, hstep, wstep)
                print("suppresion de non-maxima dans la meme couche" )
        ...
        generate_detction_txt(labels,2,image_index)
